...
import { createPreloadFragmentsBridge } from "./bridge/create-fragments-bridge";
import { createPreloadManuscriptAnnotationsBridge } from "./bridge/create-manuscript-annotations-bridge";
import { createPreloadForeshadowingBridge } from "./bridge/create-foreshadowing-bridge";
import { createPreloadActivityBridge } from "./bridge/create-activity-bridge";
import { createPreloadCharactersBridge } from "./bridge/create-characters-bridge";
import { createPreloadCanonBridge } from "./bridge/create-canon-bridge";
import { createPreloadContinuityBridge } from "./bridge/create-continuity-bridge";
import { createPreloadCharacterKnowledgeBridge } from "./bridge/create-character-knowledge-bridge";
import { createPreloadContextPlannerBridge } from "./bridge/create-context-planner-bridge";
import { createPreloadNarrativeDigestBridge } from "./bridge/create-narrative-digest-bridge";
import { createPreloadScheduleBridge } from "./bridge/create-schedule-bridge";
import { createPreloadQuickToolsBridge } from "./bridge/create-quick-tools-bridge";
import { createPreloadLoreBridge } from "./bridge/create-lore-bridge";
import { createPreloadAssistantBridge } from "./bridge/create-assistant-bridge";
import { createPreloadSettingsBridge } from "./bridge/create-settings-bridge";
import { createPreloadMusicPlaybackBridge } from "./bridge/create-music-playback-bridge";
import { createPreloadVersionBridge } from "./bridge/create-version-bridge";
import { createPreloadBackupBridge } from "./bridge/create-backup-bridge";
import { createPreloadMigrationBridge } from "./bridge/create-migration-bridge";
import { createPreloadPlotsBridge } from "./bridge/create-plots-bridge";
import { createPreloadStructureBridge } from "./bridge/create-structure-bridge";
import { createPreloadWorkspaceBridge } from "./bridge/create-workspace-bridge";
import { createPreloadSystemBridge } from "./bridge/create-system-bridge";
import { createPreloadEditorBridge } from "./bridge/create-editor-bridge";
import { createPreloadPublishingBridge } from "./bridge/create-publishing-bridge";
...
const invokeStudioBridge: BridgeInvoke = (channel, payload) => {
    if (
      ...
      channel !== MANUSCRIPT_GET_WORK_LAYOUT_SETTINGS_CHANNEL &&
      ...
      channel !== MANUSCRIPT_ANNOTATION_CREATE_CHANNEL &&
      channel !== MANUSCRIPT_ANNOTATION_LIST_CHANNEL &&
      channel !== MANUSCRIPT_ANNOTATION_UPDATE_CHANNEL &&
      channel !== MANUSCRIPT_ANNOTATION_RETIRE_CHANNEL &&
      channel !== CHARACTER_CREATE_CHANNEL &&
      channel !== CHARACTER_LIST_CHANNEL &&
      channel !== CHARACTER_UPDATE_CHANNEL &&
      channel !== CHARACTER_ADD_EVIDENCE_CHANNEL &&
      channel !== CHARACTER_RETIRE_CHANNEL &&
      channel !== CHARACTER_RELATION_CREATE_CHANNEL &&
      channel !== CHARACTER_RELATION_LIST_CHANNEL &&
      channel !== CHARACTER_RELATION_UPDATE_CHANNEL &&
      channel !== CHARACTER_RELATION_RETIRE_CHANNEL &&
      channel !== CHARACTER_EXTRACTION_RUN_CHANNEL &&
      channel !== CHARACTER_EXTRACTION_LIST_CHANNEL &&
      channel !== CHARACTER_EXTRACTION_DECIDE_CHANNEL &&
      channel !== CHARACTER_GENERATION_RUN_CHANNEL &&
      channel !== CHARACTER_GENERATION_LIST_CHANNEL &&
      channel !== CHARACTER_GENERATION_DECIDE_CHANNEL &&
      channel !== CANON_REVIEW_RUN_CHANNEL &&
      channel !== CANON_REVIEW_LIST_CHANNEL &&
      channel !== CANON_REVIEW_UPDATE_ITEM_CHANNEL &&
      channel !== CANON_REVIEW_RESOLVE_TARGET_CHANNEL &&
      channel !== CANON_REVIEW_DECIDE_CHANNEL &&
      channel !== CANON_MARKDOWN_EXPORT_CHANNEL &&
      channel !== CONTINUITY_CREATE_CHANNEL &&
      channel !== CONTINUITY_UPDATE_CHANNEL &&
      channel !== CONTINUITY_LIST_CHANNEL &&
      channel !== CONTINUITY_RESOLVE_CHANNEL &&
      channel !== CONTINUITY_DISMISS_CHANNEL &&
      channel !== CONTINUITY_REVIEW_RUN_CHANNEL &&
      channel !== CONTINUITY_REVIEW_LIST_CHANNEL &&
      channel !== CONTINUITY_REVIEW_UPDATE_ITEM_CHANNEL &&
      channel !== CONTINUITY_REVIEW_DECIDE_CHANNEL &&
      channel !== CHARACTER_KNOWLEDGE_CREATE_CHANNEL &&
      channel !== CHARACTER_KNOWLEDGE_UPDATE_CHANNEL &&
      channel !== CHARACTER_KNOWLEDGE_SUPERSEDE_CHANNEL &&
      channel !== CHARACTER_KNOWLEDGE_RETIRE_CHANNEL &&
      channel !== CHARACTER_KNOWLEDGE_LIST_CHANNEL &&
      channel !== CHARACTER_KNOWLEDGE_POV_CHANNEL &&
      channel !== CONTEXT_POLICY_LIST_CHANNEL &&
      channel !== CONTEXT_POLICY_SAVE_CHANNEL &&
      channel !== CONTEXT_PLAN_CHANNEL &&
      channel !== CONTEXT_MANIFESTS_LIST_CHANNEL &&
      channel !== CONTEXT_ACTIVITIES_LIST_CHANNEL &&
      channel !== NARRATIVE_DIGEST_GENERATE_CHANNEL &&
      channel !== NARRATIVE_DIGEST_GENERATE_SCENE_CHANNEL &&
      channel !== NARRATIVE_DIGEST_LIST_CHANNEL &&
      channel !== NARRATIVE_DIGEST_REGENERATE_CHANNEL &&
      ...
      channel !== PUBLISHING_FORM_TEMPLATE_CREATE_CHANNEL &&
      channel !== PUBLISHING_FORM_TEMPLATE_LIST_CHANNEL &&
      channel !== PUBLISHING_FORM_TEMPLATE_UPDATE_CHANNEL &&
      channel !== PUBLISHING_FORM_RESPONSE_LIST_CHANNEL &&
      channel !== PUBLISHING_FORM_RESPONSE_SAVE_CHANNEL &&
      channel !== PLOT_CREATE_CHANNEL &&
      channel !== PLOT_LIST_CHANNEL &&
      channel !== PLOT_DEFAULT_BOARD_CHANNEL &&
      channel !== PLOT_MOVE_PLACEMENT_CHANNEL &&
      channel !== PLOT_SET_STORY_TIME_CHANNEL &&
      channel !== PLOT_UPDATE_CHANNEL &&
      channel !== PLOT_RETIRE_CHANNEL &&
      channel !== PLOT_CREATE_FROM_EVENT_CHANNEL &&
      channel !== PLOT_CREATE_EVENT_CHANNEL &&
      channel !== PLOT_LINK_EVENT_CHANNEL &&
      channel !== PLOT_UNLINK_EVENT_CHANNEL &&
      channel !== PLOT_EVENT_LINK_LIST_CHANNEL &&
      ...
      channel !== MANUSCRIPT_SELECT_TEXT_IMPORT_CHANNEL &&
      ...
      channel !== MANUSCRIPT_MOVE_RANGE_TO_EPISODE_CHANNEL &&
      channel !== MANUSCRIPT_UNDO_MOVE_RANGE_TO_EPISODE_CHANNEL &&
      channel !== MANUSCRIPT_SAVE_WORK_LAYOUT_SETTINGS_CHANNEL &&
      ...
      channel !== WORKSPACE_GET_DOCUMENT_COMPLETION_CHANNEL &&
      channel !== WORKSPACE_COMPLETE_DOCUMENT_CHANNEL &&
      channel !== WORKSPACE_CLEAR_DOCUMENT_COMPLETION_CHANNEL &&
      ...
      channel !== WORKSPACE_RETIRE_ALL_DOCUMENTS_CHANNEL &&
      ...
      channel !== STRUCTURE_MOVE_EVENT_BLOCK_CHANNEL &&
      ...
      channel !== STRUCTURE_LIST_EVENT_RAIL_CHANNEL &&
      channel !== STRUCTURE_CREATE_SCENE_OVERRIDE_CHANNEL &&
      channel !== STRUCTURE_DELETE_SCENE_CHANNEL &&
      channel !== STRUCTURE_RELOCATE_SCENE_SEGMENT_CHANNEL &&
      channel !== STRUCTURE_LIST_SCENE_OVERRIDES_CHANNEL &&
      channel !== STRUCTURE_LIST_SCENE_PROJECTION_CHANNEL &&
      channel !== STRUCTURE_LIST_SCENE_CANON_CONTEXTS_CHANNEL &&
      channel !== STRUCTURE_FINALIZE_SCENE_CANON_CHECK_CHANNEL &&
      channel !== STRUCTURE_LIST_SCENE_TRASH_CHANNEL &&
      channel !== STRUCTURE_UPDATE_SCENE_RULE_SET_CHANNEL &&
      channel !== STRUCTURE_SET_SCENE_EVENT_OVERRIDE_CHANNEL &&
      channel !== STRUCTURE_REBIND_SCENE_METADATA_CHANNEL &&
      channel !== STRUCTURE_PREPARE_SCENE_DELETION_CHANNEL &&
      channel !== STRUCTURE_RESTORE_SCENE_TRASH_CHANNEL &&
      channel !== STRUCTURE_UNDO_SCENE_DELETION_CHANNEL &&
      channel !== STRUCTURE_RUN_SCENE_EXTRACTION_CHANNEL &&
      channel !== STRUCTURE_LIST_SCENE_EXTRACTION_CANDIDATES_CHANNEL &&
      channel !== STRUCTURE_DECIDE_SCENE_EXTRACTION_BOUNDARY_CHANNEL &&
      channel !== STRUCTURE_LIST_SCENE_ANNOTATIONS_CHANNEL &&
      channel !== STRUCTURE_DECIDE_SCENE_EXTRACTION_ANNOTATION_CHANNEL &&
      channel !== STRUCTURE_RUN_SCENE_DRAFT_CHANNEL &&
      channel !== STRUCTURE_LIST_SCENE_DRAFT_CANDIDATES_CHANNEL &&
      channel !== STRUCTURE_UPDATE_SCENE_DRAFT_CANDIDATE_CHANNEL &&
      channel !== STRUCTURE_PREPARE_SCENE_DRAFT_INSERTION_CHANNEL &&
      channel !== STRUCTURE_COMPLETE_SCENE_DRAFT_INSERTION_CHANNEL &&
      ...
      channel !== ACTIVITY_UPDATE_POMODORO_NOTE_CHANNEL &&
      channel !== ACTIVITY_STOP_POMODORO_CHANNEL &&
      channel !== SCHEDULE_LIST_WORK_CHANNEL &&
      channel !== SCHEDULE_LIST_CALENDAR_CHANNEL &&
      channel !== SCHEDULE_LIST_TODAY_CHANNEL &&
      ...
      channel !== ASSISTANT_CANCEL_REQUEST_CHANNEL &&
      channel !== ASSISTANT_CHATGPT_OAUTH_START_LOGIN_CHANNEL &&
      channel !== ASSISTANT_CHAT_RUN_CHANNEL &&
      ...
      channel !== UI_PREFERENCES_GET_CHANNEL &&
      channel !== UI_PREFERENCES_SAVE_CHANNEL &&
      channel !== MUSIC_SETTINGS_PROFILE_CHANNEL &&
      channel !== MUSIC_SETTINGS_GET_WORK_CHANNEL &&
      channel !== MUSIC_SETTINGS_SAVE_WORK_CHANNEL &&
      channel !== SCENE_ANALYSIS_SETTINGS_GET_WORK_CHANNEL &&
      channel !== SCENE_ANALYSIS_SETTINGS_SAVE_WORK_CHANNEL &&
      channel !== INSPIRATION_SETTINGS_GET_WORK_CHANNEL &&
      channel !== INSPIRATION_SETTINGS_SAVE_WORK_CHANNEL &&
      channel !== YOUTUBE_MUSIC_CONNECTION_STATUS_CHANNEL &&
      channel !== YOUTUBE_MUSIC_CONNECTION_SAVE_CHANNEL &&
      channel !== YOUTUBE_MUSIC_PROFILE_CHANNEL &&
      channel !== YOUTUBE_MUSIC_SEARCH_CHANNEL &&
      channel !== LOCAL_MEDIA_INSPECT_CHANNEL &&
      channel !== LOCAL_MEDIA_RELINK_CHANNEL &&
      channel !== LOCAL_MEDIA_SELECT_CHANNEL &&
      channel !== SCENE_MUSIC_QUEUE_SEARCH_CHANNEL &&
      channel !== SCENE_MUSIC_QUEUE_LIST_CHANNEL &&
      channel !== SCENE_MUSIC_QUEUE_SELECT_CHANNEL &&
      channel !== VERSION_LIST_DOCUMENT_REVISIONS_CHANNEL &&
      channel !== VERSION_READ_DOCUMENT_REVISION_CHANNEL &&
      ...
      channel !== VERSION_PLAN_WORK_SNAPSHOT_SCENES_CHANNEL &&
...
const listenStudioBridge: BridgeListen = (channel, listener) => {
    if (
      channel !==
      MANUSCRIPT_CLOSE_REQUEST_CHANNEL
    ) {
...
const studioBridge = createStudioBridge(
  ...
  listenStudioBridge,
  {
    activity: createPreloadActivityBridge(invokeStudioBridge),
    assistant: createPreloadAssistantBridge(invokeStudioBridge),
    backup: createPreloadBackupBridge(invokeStudioBridge),
    editor: createPreloadEditorBridge(invokeStudioBridge, listenStudioBridge),
    characters: createPreloadCharactersBridge(invokeStudioBridge),
    canon: createPreloadCanonBridge(invokeStudioBridge),
    continuity: createPreloadContinuityBridge(invokeStudioBridge),
    characterKnowledge: createPreloadCharacterKnowledgeBridge(invokeStudioBridge),
    contextPlanner: createPreloadContextPlannerBridge(invokeStudioBridge),
    narrativeDigest: createPreloadNarrativeDigestBridge(invokeStudioBridge),
    fragments: createPreloadFragmentsBridge(invokeStudioBridge),
    manuscriptAnnotations:
      createPreloadManuscriptAnnotationsBridge(invokeStudioBridge),
    ...
    settings: createPreloadSettingsBridge(invokeStudioBridge),
